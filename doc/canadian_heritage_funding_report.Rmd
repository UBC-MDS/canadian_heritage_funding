...
Therefore, we selected random forest as the best performing model to conduct hyperparameter optimization and tuned `max_features` in `CountVectorizer()`, and `max_depth" and "class_weight` as well as the `max_features` argument in `RandomForestClassifier()`.   
